...
const LoginDialog = ({ trigger, autoOpen = false, showSettings = true, info, onLoginSuccess }: LoginDialogProps) => {
  ...
  const isFormValid = passwordLoginEnabled && username.trim() !== "" && password.trim() !== "";
    ...
    if (loading) {
      return <Button color="gray" variant="soft" disabled>{t("loading")}</Button>;
    }
    if (error || !account) {
      return <Button disabled color="gray" variant="soft">Error</Button>;
    }
    ...
    if (onlyOAuthLogin && !autoOpen) {
      ...
      if (trigger) {
        ...
        if (typeof trigger === "string") {
          return <Button color="gray" variant="soft" onClick={redirect}>{trigger}</Button>;
        }
        return (
      ...
      // 默认按钮
      return <Button color="gray" variant="soft" onClick={redirect}>{t("login.title")}</Button>;
    }
    return (
